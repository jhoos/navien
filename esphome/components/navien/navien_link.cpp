...
bool NavienCommand::change_observed(const RECV_BUFFER& recv_buffer, uint8_t recv_len) const {
  if (this->idx >= recv_len)
...
uint8_t NavienCommand::send(NavienUartI* uart) {
...
void NavienLink::check_command_complete() {
  if (!this->cmd_buffer.empty()){
    ...
    if (cmd_buffer.front().change_observed(this->recv_buffer, len + HDR_SIZE)){
      // The queued command has been processed. Replace the first command in the buffer
      // with an acknowledgement, so that the acknowledgement will be sent next, before 
      // any other pending commands.
      ESP_LOGV(TAG, "Command response received, sending acknowledgement");
      cmd_buffer.front() = NavienCommand(
...
void NavienLink::send_queued_command() {
  if (!this->cmd_buffer.empty()){
    // Send the command
    NavienCommand &cmd = cmd_buffer.front();
...
void NavienLink::send_dhw_set_temp_cmd(float temp) {
  ...
  send_cmd(NavienCommand(
    ...
    0xFF, static_cast<uint8_t>(temp * 2 + 0.5),
    20 // retry longer because sometimes the unit takes longer to respond to this command
  )); 
}
